...
rule emu:
    ...
    shell: "emu abundance {input.reads} --type map-pb --db {input.db} --output-dir {work_dir}/emu/{wildcards.sample} --output-basename {wildcards.sample} --keep-files --keep-counts --keep-read-assignments --output-unclassified --threads {threads}"
...
rule chopper:
    ...
    shell:
        "gunzip -c {input.reads} | chopper -q 10 -l 1300 --maxlength 1600 | gzip > {output}"	

rule make temp_dir:
    input: 
        reads = "{work_dir}/fastq_pass/{sample}.fastq.gz"
    output: 
